...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var API_REACTIVATE = "/conversation-stats/api/reactivate";
    ...
    var st = {
      ...
      loadBtn: { border: "1px solid rgba(52,211,153,.5)", background: "rgba(52,211,153,.10)", borderRadius: 5, padding: "2px 8px", fontSize: fs(11), cursor: "pointer", color: "inherit" },
    ...
    // 恢复归档会话：从 DSH workspace 归档集合中移除该 id（GET，同上）。
    async function reactivateSession(id) {
      var res = await fetch(API_REACTIVATE + "?id=" + encodeURIComponent(id), { cache: "no-store" });
      var data = null;
      try { data = await res.json(); } catch (e) { data = null; }
      if (!res.ok || !data || data.ok === false) {
        throw new Error((data && data.error) || "HTTP " + res.status + " 恢复失败");
      }
      return data;
    }
    ...
    function SummaryTable(props) {
      ...
      var heads = ["会话", "轮数", "步数", "调用", "输出tok", "输入tok", "缓存tok", "命中率", "最后活跃", "加载", "删除"];
      var rows = sessions.map(function (s) {
        ...
        // 缓存命中率：命中读取 /（新增输入 + 命中读取）
        var hitDenom = (s.inputTokens || 0) + (s.cacheReadTokens || 0);
        var hitRate = hitDenom > 0 ? ((s.cacheReadTokens || 0) / hitDenom) * 100 : null;
        return el("tr", {
          key: s.id,
          ...
          el("td", { style: st.td }, fmtPercent(hitRate)),
          el("td", { style: st.td }, fmtTime(s.lastTime)),
          el("td", { style: st.td, textAlign: "center" }, s.archived ? el("button", {
            style: st.loadBtn,
            title: "加载该归档对话回 DSH 对话列表（需重启 DSH Web 生效）",
            onClick: function (e) { e.stopPropagation(); props.onReactivate && props.onReactivate(s); }
          }, "恢复") : "—"),
    ...
    function StatsPanel() {
      ...
      var setM = meta[1];
      // 顶部轻提示（恢复成功/失败等临时消息）
      var note = useState(null); // { text, kind }
      var noteVal = note[0];
      var setNote = note[1];
      ...
      // ── 恢复归档会话：移除归档标记 → 刷新 ──
      function runReactivate(s) {
        if (!s || !s.id || !s.archived) return;
        reactivateSession(s.id)
          .then(function (res) {
            setNote({ text: (res && res.message) || "已恢复该会话（重启 DSH Web 后在对话列表生效）", kind: "ok" });
            setTimeout(function () { setNote(null); }, 5000);
            loadList(true);
          })
          .catch(function (e) {
            setNote({ text: "恢复失败：" + String((e && e.message) || e), kind: "err" });
            setTimeout(function () { setNote(null); }, 6000);
          });
      }
      ...
      if (m.error !== null && !data.loaded) {
        ...
        body = el("div", {},
          ...
          el(SummaryTable, { sessions: data.sessions, onPick: openDetail, selectedId: null, onDelete: openDelete, onReactivate: runReactivate }),
      ...
      var noteEl = noteVal ? el("div", { style: noteVal.kind === "err"
        ? Object.assign({}, st.err, { margin: "0 0 10px" })
        : { padding: "8px 12px", borderRadius: 8, background: "rgba(52,211,153,.12)", border: "1px solid rgba(52,211,153,.35)", fontSize: fs(11), margin: "0 0 10px" }
      }, noteVal.text) : null;

      return el("div", { style: st.wrap }, el("div", { style: st.root }, head, noteEl, body, modal));
    }
