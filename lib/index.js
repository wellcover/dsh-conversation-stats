...
export default {
  ...
  apply(ctx) {
    ...
    try {
      ...
      /**
       * Read the set of ARCHIVED session ids from DSH's workspace storage
       * (`storages/workspace.json → global.archivedSessionIds`). The DSH front
       * end hides exactly these sessions from the conversation list, so
       * "reloading" an archived conversation means removing its id here.
       */
      async function archivedSessionIds() {
        try {
          const j = JSON.parse(await readFile(join(storagesRoot(), 'workspace.json'), 'utf8'))
          const arr = j && j.global && Array.isArray(j.global.archivedSessionIds) ? j.global.archivedSessionIds : []
          return arr
        } catch (e) {
          return []
        }
      ...
      async function scanAll() {
        ...
        const archived = new Set(await archivedSessionIds())
        const list = []
        let count = 0
      ...
      /**
       * "Reload" an archived conversation: remove its id from
       * `global.archivedSessionIds` in DSH's `storages/workspace.json` so the
       * DSH conversation list shows it again. Only touches that one array;
       * every other key (unit.version, workspaces, etc.) is preserved as-is.
       */
      async function reactivateSession(sessionId) {
        if (!/^\w[\w-]*$/.test(sessionId)) return { ok: false, error: '非法会话 id' }
        const file = join(storagesRoot(), 'workspace.json')
        let j
        try {
          j = JSON.parse(await readFile(file, 'utf8'))
        } catch (e) {
          return { ok: false, error: '读取 workspace.json 失败：' + msg(e) }
        }
        if (!j || !j.global || !Array.isArray(j.global.archivedSessionIds)) {
          return { ok: false, error: 'workspace.json 缺少 global.archivedSessionIds' }
        }
        if (!j.global.archivedSessionIds.includes(sessionId)) {
          return { ok: true, id: sessionId, changed: false, message: '该会话不在归档列表（可能已恢复）' }
        }
        j.global.archivedSessionIds = j.global.archivedSessionIds.filter((x) => x !== sessionId)
        await writeFile(file, JSON.stringify(j, null, 2) + '\n', 'utf8')
        return { ok: true, id: sessionId, changed: true, message: '已恢复，重启 DSH Web 后在对话列表生效' }
      }
      ...
      if (webServer && typeof webServer.register === 'function') {
        try {
          ...
          webServer.register({
            kind: 'exact',
            path: '/conversation-stats/api/reactivate',
            handler: async (req, res) => {
              const query = queryOf(req)
              const id = query.get('id') || ''
              let payload
              try {
                if (!id) payload = { ok: false, error: '缺少会话 id' }
                else payload = await reactivateSession(id)
              } catch (e) {
                payload = { ok: false, error: msg(e) }
              }
              sendJson(res, payload)
          ...
          push('route-registered:reactivate')
        } catch (e) {
